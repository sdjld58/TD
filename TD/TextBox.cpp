...
void UI::TextBox::sideDraw(sf::RenderWindow& window) {
    ...
    if (uip->curWaveIsDefense) {
        ...
        for (int unitId = 1; unitId <= 9; ++unitId) {
            ...
            std::string unitCost;
            int alpha = 255; // 기본 투명도

            // judgeStage에 따른 투명도 조정
            if ((uip->judgeStage == 1 && (unitId == 7 || unitId == 8 || unitId == 9)) ||
                (uip->judgeStage == 2 && (unitId == 8 || unitId == 9))) {
                alpha = 100; // 투명하게 설정
            }
            ...
            // 투명도 적용
            sf::Color spriteColor = unitSprite.getColor();
            spriteColor.a = alpha;
            unitSprite.setColor(spriteColor);

            // 이름 텍스트 생성
            sf::Text nameText(unitName, fontbm, 20);
            nameText.setFillColor(sf::Color(255, 255, 255, alpha)); // 텍스트에 투명도 적용
            nameText.setPosition(currentX - xAdjustment, currentY - 30.f);

            // 골드 텍스트 생성
            sf::Text costText(unitCost, fontbm, 20);
            costText.setFillColor(sf::Color(255, 255, 0, alpha)); // 텍스트에 투명도 적용
            costText.setPosition(currentX - xAdjustment, currentY - 5.f);
